...
%%
% Book metadata
\title{ {\Large CalculoNumerico.jl- uma abordagem  computacional \\ do cálculo numérico}}
\author[]{ {\small Emerson Vitor Castelani \&  Rodrigo Schulz}}
\publisher{\footnotesize Universidade Estadual de Maringá  \&  Universidade Técnologica Federal do Paraná}
...
%f the License at \url{http://www.apache.org/licenses/LICENSE-2.0}. Unless
%equired by applicable law or agreed to in writing, software distributed
%nder the License is distributed on an \smallcaps{``AS IS'' BASIS, WITHOUT
...
%nder the License.\index{license}

\par\textit{Primeira impressão, \monthyear}
\end{fullwidth}
...
\nohyphenation
Dedicado a todos os interessados na leitura do tema aqui abordado. 
\end{doublespace}
\vfill
\vfill
...
O presente livro é um protótipo de notas de aula sobre a disciplina de  Cálculo Numérico. Existem diversos materiais que, certamente, são mais completos que este. A principal diferença de nosso material em relação ao material desenvolvido por outros profissionais é que juntos atralemos um módulo chamado \texttt{CalculoNumerico.jl} que pode ser instalado, modificado, compartilhado, etc. Este módulo é desenvolvido em Linguagem Julia \cite{bezanson2017julia
}, de fácil instalação e permite acesso rápido aos algoritmos aqui apresentados. 


\chapter{Resolução de equações reais de uma variável}

Nesta seção estamos interessados em elucidar alguns conceitos e métodos relacionados ao problema de se encontrar uma raiz de uma função $f:\mathbb{R}\rightarrow \mathbb{R}$. A equação definida pela função $f$ pode ser algébrica ou transcedente. Os assuntos que iremos tratar serão os seguintes:

\begin{enumerate}
	\item  Conceitos elementares;
	\item  Método da bipartição;
	\item Método de falsa posição;
	\item Método das aproximações sucessivas;
	\item Método de Newton;
	\item Método da Secante;
	\item Caso particular, polinômios;
\end{enumerate}

\subsection{Conceitos elementares}
A formulação mais simples deste problema é considerar equações simples como uma equação linear dada no exemplo a seguir
$$2x+1=0.$$

Evidentemente encontrar a solução de uma equação como esta é simples e não é necessário utilizar qualquer algoritmo para encontrar a resposta $x=\dfrac{1}{2}$. O mesmo pode ser dito quando consideramos equações do segundo grau. Fórmulas simples e fechadas podem nos conduzir rapidamente à resposta do problema (tais estratégias são chamadas métodos diretos). Contudo, numa situação mais geral esta tarefa pode ser complicada de se resolver sem apelar ao uso de um método numérico (normalmente iterativo).

O problema que estamos tentando resolver aqui é encontrar $x\in\mathbb{R}$ tal que $f(x)=0$. Naturalmente este problema generaliza os exemplos dados.

Neste contexto, surge uma pergunta natural: será que é possível estabelecer um método ou estratégia capaz de resolver facilmente este problema que propomos?

A resposta desta pergunta depende muito das "informações" que temos em mão. Estas informações podem ser inclusive, características da propria função $f$ como por exemplo, continuidade e diferenciabilidade, ou ainda, estimativas de intervalos contendo a raíz, etc.

No intuito de resolver este problema, a primeira saída consiste em apelarmos a geometria do problema e visualizarmos o gráfico da função na busca de encontrar uma solução para nosso problema. Vejamos um exemplo. Defina no REPL a seguinte função:
